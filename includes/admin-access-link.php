...
add_action('rest_api_init', callback: 'agent2wp_register_admin_access_route');

/**
 * Register the REST endpoints used by temporary admin access.
 */
function agent2wp_register_admin_access_route(): void
{
    register_rest_route(route_namespace: 'agent2wp/v1', route: '/admin-access', args: [
        'methods' => ['POST'],
        'callback' => 'agent2wp_handle_admin_access_exchange',
        'permission_callback' => '__return_true',
    ]);

    register_rest_route(route_namespace: 'agent2wp/v1', route: '/admin-access/(?P<nonce>[A-Za-z0-9_-]+)', args: [
        'methods' => ['GET'],
        'callback' => 'agent2wp_handle_admin_access_login',
        'permission_callback' => '__return_true',
    ]);
}
 ...
 */
function agent2wp_create_admin_access_token(
    int $user_id,
    int $expires_in,
    ...
    if (!$user instanceof WP_User || !agent2wp_user_can_manage($user)) {
        return new WP_Error('invalid_admin_access_user', 'Admin access links can only be created for administrators.');
    }

    $redirect_url = agent2wp_resolve_admin_access_redirect($admin_path);
    ...
    $payload = [
        ...
        'nonce_hash' => agent2wp_admin_access_nonce_hash($nonce),
    ];

    if (!set_transient(agent2wp_admin_access_transient_key($token), $payload, $expires_in)) {
 ...
 */
function agent2wp_handle_admin_access_exchange(WP_REST_Request $request)
{
    if (!agent2wp_is_enabled()) {
        return new WP_Error('agent2wp_disabled', 'Agent2Wp abilities are disabled.', ['status' => 403]);
    }

    $token = agent2wp_get_admin_access_token_from_request($request);
    if ($token === '') {
        return new WP_Error('missing_admin_access_token', 'Missing admin access token.', ['status' => 401]);
    }

    $nonce = agent2wp_get_admin_access_nonce_from_request($request);
    if ($nonce === '') {
        return new WP_Error('missing_admin_access_nonce', 'Missing admin access nonce.', ['status' => 401]);
    }

    /** @var mixed $payload */
    $payload = get_transient(agent2wp_admin_access_transient_key($token));
    delete_transient(agent2wp_admin_access_transient_key($token));
    ...
    if (!hash_equals($payload['nonce_hash'], agent2wp_admin_access_nonce_hash($nonce))) {
        return new WP_Error('invalid_admin_access_token', 'Invalid or expired admin access token.', ['status' => 401]);
    }

    $access = agent2wp_validate_admin_access_payload($payload);
    ...
    if (!set_transient(
        agent2wp_admin_access_login_nonce_transient_key($login_nonce),
        $login_payload,
        $login_expires_in,
    )) {
    ...
    $response = new WP_REST_Response([
        'login_url' => rest_url('agent2wp/v1/admin-access/' . rawurlencode($login_nonce)),
        'expires_at' => $login_expires_at,
        'session_expires_in' => $access['session_expires_in'],
        'redirect_url' => $access['redirect_url'],
        'one_time' => true,
    ]);
    agent2wp_admin_access_no_store_headers($response);
 ...
 * @return WP_REST_Response|WP_Error
 */
function agent2wp_handle_admin_access_login(WP_REST_Request $request)
{
    if (!agent2wp_is_enabled()) {
        return new WP_Error('agent2wp_disabled', 'Agent2Wp abilities are disabled.', ['status' => 403]);
    }
    ...
    /** @var mixed $payload */
    $payload = get_transient(agent2wp_admin_access_login_nonce_transient_key($nonce));
    delete_transient(agent2wp_admin_access_login_nonce_transient_key($nonce));
    ...
    return agent2wp_create_admin_access_redirect_response($access);
}

/**
 * Return the admin access token from request headers.
 */
function agent2wp_get_admin_access_token_from_request(WP_REST_Request $request): string
{
    return agent2wp_rest_header_token($request, header_name: 'x-agent2wp-admin-access-token');
}

/**
 * Return the admin access binding nonce from request headers.
 */
function agent2wp_get_admin_access_nonce_from_request(WP_REST_Request $request): string
{
    $header_nonce = $request->get_header('x-agent2wp-admin-access-nonce');
 ...
 */
function agent2wp_validate_admin_access_payload(array $payload): array|WP_Error
{
 ...
 */
function agent2wp_create_admin_access_redirect_response(array $access): WP_REST_Response
{
    $session_expires_in = $access['session_expires_in'];
 ...
 */
function agent2wp_admin_access_no_store_headers(WP_REST_Response $response): void
{
 ...
 */
function agent2wp_resolve_admin_access_redirect(string $admin_path)
{
 ...
 */
function agent2wp_admin_access_transient_key(string $token): string
{
    return 'agent2wp_admin_access_' . hash_hmac('sha256', $token, wp_salt('auth'));
}

/**
 * Return the transient key for a one-time admin access browser nonce.
 */
function agent2wp_admin_access_login_nonce_transient_key(string $nonce): string
{
    return 'agent2wp_admin_access_login_' . hash_hmac('sha256', $nonce, wp_salt('auth'));
}

/**
 * Return the stored hash for an admin access binding nonce.
 */
function agent2wp_admin_access_nonce_hash(string $nonce): string
{
    return hash_hmac('sha256', $nonce, wp_salt('nonce') . '|agent2wp-admin-access');
}
