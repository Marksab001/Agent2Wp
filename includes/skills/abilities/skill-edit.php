...
namespace Agent2Wp\Skills\Abilities\SkillEdit;
...
function register(): void
{
    ...
    wp_register_ability('agent2wp/skill-edit', [
        'label' => __('Edit Skill', domain: 'agent2wp'),
        'description' => __(
...
function execute(array $input): array|WP_Error
{
    ...
    if (array_key_exists('title', $input)) {
        ...
        if ($new_title !== $post->post_name) {
            ...
            if ($clash !== null && (int) $clash->ID !== (int) $post->ID) {
                return new WP_Error('slug_exists', __('A skill with this title already exists.', domain: 'agent2wp'));
            }
            wp_update_post([
                'ID' => $post->ID,
    ...
    if (array_key_exists('description', $input)) {
        ...
        if ($new_description === '') {
            return new WP_Error('missing_description', __('Description cannot be empty.', domain: 'agent2wp'));
        }
    ...
    if (array_key_exists('content', $input)) {
        ...
        if (strlen($new_content) > Parser\MAX_BODY_BYTES) {
            return new WP_Error('body_too_large', __('Body exceeds 1 MB.', domain: 'agent2wp'));
        }
