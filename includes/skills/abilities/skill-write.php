...
namespace Agent2Wp\Skills\Abilities\SkillWrite;
...
function register(): void
{
    ...
    wp_register_ability('agent2wp/skill-write', [
        'label' => __('Write Skill', domain: 'agent2wp'),
        'description' => __(
            'Create or update a Agent2Wp user skill. The `title` is the only identifier — it is sanitised server-side (lowercase, dash-separated) and becomes both the human label and the slug used for lookups.',
            domain: 'agent2wp',
        ),
        'category' => 'skill',
        'input_schema' => [
    ...
    if ($external_label !== null) {
        return new WP_Error(
            ...
            sprintf(
                ...
                __('Slug "%1$s" is already used by source "%2$s". Choose a different title.', domain: 'agent2wp'),
                $title,
                $external_label,
            ),
