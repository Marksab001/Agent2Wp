...
namespace Agent2Wp\Skills\Abilities;
...
function register_categories(): void
{
    ...
    try {
        wp_register_ability_category('skill', [
            'label' => __('Skills', domain: 'agent2wp'),
            'description' => __('Manage and load Agent2Wp skills.', domain: 'agent2wp'),
        ]);
...
namespace Agent2Wp\Skills\Abilities\SkillGet;
...
/**
 * Take canonical ownership of `agent2wp/skill-get`. Registered at
 ...
 * (`agent2wp_skill_lookup_sources`), the fallback path stops firing
...
function register(): void
{
    ...
    $previous = wp_get_ability('agent2wp/skill-get');
    if ($previous instanceof \WP_Ability) {
        wp_unregister_ability('agent2wp/skill-get');
    }

    wp_register_ability('agent2wp/skill-get', [
        'label' => __('Get Skill', domain: 'agent2wp'),
        'description' => __(
            'Load a Agent2Wp skill by slug. Returns the full SKILL.md content plus metadata.',
            domain: 'agent2wp',
        ),
        'category' => 'skill',
        'input_schema' => [
        ...
        'execute_callback' => static function (array $input) use ($previous): array|WP_Error {
            ...
            if ($agent_slug === '') {
                return new WP_Error('missing_slug', __('A slug is required.', domain: 'agent2wp'));
            }
...
function normalize_requested_slug(string $slug): string
{
    ...
    if (str_starts_with($normalized, 'agent2wp/')) {
        return substr($normalized, strlen('agent2wp/'));
    }
