...
namespace Agent2Wp\Skills\Notices;
...
const RELOAD_NOTICE_TRANSIENT = 'agent2wp_skill_pending_reload_notice';

const RELOAD_NOTICE_TTL = 60;

/**
 * Mark that the skill index has changed and a reload-MCP-client notice
 * should be shown to the admin the next time they land on a Agent2Wp
 ...
 *    current admin screen belongs to Agent2Wp.
 */
function render(): void
{
    ...
    $inline_key = 'agent2wp_skill_admin_notice_' . get_current_user_id();
    ...
    // and we're on the Skills admin page specifically — other Agent2Wp
    // screens don't need this prompt.
    $screen = function_exists('get_current_screen') ? get_current_screen() : null;
    if (!$screen instanceof \WP_Screen) {
        return;
    }
    if ($screen->id !== 'agent2wp_page_' . Admin\PAGE_SLUG) {
        return;
    }
