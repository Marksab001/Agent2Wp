...
namespace Agent2Wp\Skills\Admin;

use Agent2Wp\Skills\Abilities\SkillWrite;
use Agent2Wp\Skills\Cpt;
use Agent2Wp\Skills\Notices;
...
const PAGE_SLUG = 'agent2wp-skills';

function capability(): string
{
    return \agent2wp_manage_capability();
}
...
/**
 * Skills lives as a submenu under the Agent2Wp top-level menu registered
 * in `agent2wp.php`. The parent slug `agent2wp-connect` is the canonical
...
function register_menu(): void
{
    add_submenu_page(
        ...
        page_title: __('Skills', domain: 'agent2wp'),
        menu_title: __('Skills', domain: 'agent2wp'),
 ...
 * Abilities Hub (slug `agent2wp-abilities`), in the order Context then Skills,
 * inside the Agent2Wp menu group. Runs at a priority higher than any caller of
...
{
    global $submenu;
    if (!is_array($submenu ?? null) || !is_array($submenu['agent2wp-connect'] ?? null)) {
        return;
    }

    /** @var array<int, array<int, string>> $entries */
    $entries = $submenu['agent2wp-connect'];
    ...
    foreach ($entries as $entry) {
        $reordered[] = $entry;
        if (!$inserted && ($entry[2] ?? null) === 'agent2wp-abilities') {
    ...
    $submenu['agent2wp-connect'] = $reordered;
}

function render_page(): void
{
    if (!current_user_can_manage()) {
        wp_die(__('You do not have permission to manage skills.', domain: 'agent2wp'));
    }
    $view = agent2wp_admin_request_get_string('skill') !== '' ? 'edit' : 'list';
    if ($view === 'edit') {
...
{
    $actions = [
        'agent2wp_skill_create',
        'agent2wp_skill_update',
        'agent2wp_skill_delete',
        'agent2wp_skill_restore',
        'agent2wp_skill_permanent_delete',
        'agent2wp_skill_toggle_status',
        'agent2wp_skill_toggle_activation',
        'agent2wp_skill_upload',
        'agent2wp_skill_download',
        'agent2wp_skill_download_all',
    ];
    foreach ($actions as $action) {
        $handler = 'handle_' . substr($action, strlen('agent2wp_skill_'));
...
function enqueue_assets(string $hook): void
{
    ...
    // the sanitised menu_title of the parent top-level menu ("Agent2Wp").
    if ($hook !== 'agent2wp_page_' . PAGE_SLUG) {
        return;
    }
    wp_enqueue_style(
        'agent2wp-skills-admin',
        (string) AGENT2WP_PLUGIN_URL . 'includes/skills/assets/admin.css',
        [],
        AGENT2WP_VERSION,
    );
    ...
    if (agent2wp_admin_request_get_string('skill') === '') {
        return;
    }
    ...
    wp_add_inline_script('code-editor', sprintf(
        'jQuery(function($){ var el = document.getElementById("agent2wp-skills-content"); if (el && window.wp && wp.codeEditor) { var inst = wp.codeEditor.initialize(el, %s); window.agent2wpSkillsEditor = inst && inst.codemirror ? inst.codemirror : null; window.dispatchEvent(new CustomEvent("agent2wp-skills-editor-ready")); } });',
...
{
    require_capability_and_nonce('agent2wp_skill_create');
...
function handle_update(): void
{
    ...
    require_capability_and_nonce('agent2wp_skill_update_' . $post_id);
    ...
    if ($new_title === '') {
        redirect_with_notice(
            'error',
            __('Title must contain at least one letter or digit (lowercase, dash-separated).', domain: 'agent2wp'),
            $post_id,
        );
        return;
    }
    ...
    if (trim($input['description']) === '') {
        redirect_with_notice('error', __('Description is required.', domain: 'agent2wp'), $post_id);
        return;
    }

    $content = Parser\unescape_content($input['content']);
    if (strlen($content) > Parser\MAX_BODY_BYTES) {
        redirect_with_notice('error', __('Body exceeds 1 MB.', domain: 'agent2wp'), $post_id);
        return;
    }
    ...
    if ($new_title !== $post->post_name) {
        ...
        if ($external !== null) {
            redirect_with_notice(
                ...
                sprintf(
                    ...
                    __('Title "%1$s" is already used by source "%2$s". Choose a different title.', domain: 'agent2wp'),
                    $new_title,
                    $external,
                ),
                $post_id,
        ...
        if ($clash !== null && (int) $clash->ID !== $post_id) {
            redirect_with_notice('error', __('A skill with this title already exists.', domain: 'agent2wp'), $post_id);
            return;
        }
    }
...
{
    $post_id = read_post_id();
    require_capability_and_nonce('agent2wp_skill_delete_' . $post_id);
...
{
    $post_id = read_post_id();
    require_capability_and_nonce('agent2wp_skill_restore_' . $post_id);
...
{
    $post_id = read_post_id();
    require_capability_and_nonce('agent2wp_skill_permanent_delete_' . $post_id);
...
{
    $post_id = read_post_id();
    require_capability_and_nonce('agent2wp_skill_toggle_status_' . $post_id);
    $post = load_skill_post($post_id);
...
function handle_toggle_activation(): void
{
    ...
    require_capability_and_nonce('agent2wp_skill_toggle_activation_' . $post_id);
    if (!in_array($field, ['enable_prompt', 'enable_agentic'], strict: true)) {
        wp_die(__('Invalid field.', domain: 'agent2wp'));
    }
...
{
    require_capability_and_nonce('agent2wp_skill_upload');

    $files = normalize_uploaded_files($_FILES['skill_file'] ?? null);
    if ($files === []) {
        redirect_with_notice('error', __('No files uploaded.', domain: 'agent2wp'));
        return;
    }
    if (count($files) > MAX_UPLOAD_FILES) {
        redirect_with_notice('error', sprintf(
            ...
            __('Too many files. Upload up to %d skills at a time.', domain: 'agent2wp'),
            MAX_UPLOAD_FILES,
        ));
        return;
    }
    ...
    if ($imported !== []) {
        ...
        if ($imported_ids !== []) {
            set_transient('agent2wp_skill_just_imported_' . get_current_user_id(), $imported_ids, expiration: 30);
        }
    }
...
function process_one_upload(array $file, string $on_conflict): string|\WP_Error
{
    if ($file['error'] !== UPLOAD_ERR_OK) {
        return new \WP_Error('upload_error', __('upload failed', domain: 'agent2wp'));
    }
    if (!str_ends_with(strtolower($file['name']), '.md')) {
        return new \WP_Error('not_md', __('not a .md file', domain: 'agent2wp'));
    }
    if ($file['size'] > Parser\MAX_BODY_BYTES) {
        return new \WP_Error('too_large', __('too large (max 1 MB)', domain: 'agent2wp'));
    }
    if ($file['tmp_name'] === '' || !is_readable($file['tmp_name'])) {
        return new \WP_Error('unreadable', __('could not be read', domain: 'agent2wp'));
    }
...
function build_upload_notice(array $imported, array $failed): string
{
    if ($imported === [] && $failed !== []) {
        return sprintf(
            ...
            __('Upload failed: %s', domain: 'agent2wp'),
            implode(', ', $failed),
        );
    }
    $head = sprintf(
        /* translators: %d: count */
        _n('%d skill imported.', plural: '%d skills imported.', number: count($imported), domain: 'agent2wp'),
        count($imported),
    );
    if ($failed === []) {
        return $head;
    ...
    return sprintf(
        ...
        __('%1$s Failed: %2$s', domain: 'agent2wp'),
        $head,
        implode(', ', $failed),
    );
}
...
{
    $post_id = read_post_id();
    require_capability_and_nonce('agent2wp_skill_download_' . $post_id);
    $post = load_skill_post($post_id);
...
{
    require_capability_and_nonce('agent2wp_skill_download_all');
    ...
    if ($posts === []) {
        redirect_with_notice('error', __('No skills to download.', domain: 'agent2wp'));
        return;
    }

    $tmp = wp_tempnam('agent2wp-skills-' . time() . '.zip');
    if ($tmp === '') {
        redirect_with_notice('error', __('Could not create temporary ZIP file.', domain: 'agent2wp'));
        return;
    }
    ...
    if ($zip->open($tmp, \ZipArchive::OVERWRITE) !== true) {
        wp_delete_file($tmp);
        redirect_with_notice('error', __('Could not open the ZIP file for writing.', domain: 'agent2wp'));
        return;
    }
    ...
    wp_delete_file($tmp);
    if ($raw === false) {
        redirect_with_notice('error', __('Could not read the generated ZIP.', domain: 'agent2wp'));
        return;
    }

    $filename = sprintf('agent2wp-skills-%s.zip', wp_date('Y-m-d'));
    nocache_headers();
...
{
    if (!current_user_can_manage()) {
        wp_die(__('Not allowed.', domain: 'agent2wp'), title: '', args: ['response' => 403]);
    }
...
function read_post_id(): int
{
    if (isset($_POST['post_id'])) {
        $raw = $_POST['post_id'];
        return is_scalar($raw) ? (int) wp_unslash((string) $raw) : 0;
    }

    return (int) agent2wp_admin_request_get_string('post_id', '0');
}
...
{
    set_transient(
        'agent2wp_skill_admin_notice_' . get_current_user_id(),
