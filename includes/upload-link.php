...
add_action('rest_api_init', callback: 'agent2wp_register_upload_route');

/**
 * Register the REST endpoint used by signed uploads.
 */
function agent2wp_register_upload_route(): void
{
    $route_namespace = 'agent2wp/v1';
    $route = '/upload';

    register_rest_route($route_namespace, $route, [
        'methods' => ['POST', 'PUT'],
        'callback' => 'agent2wp_handle_signed_upload',
 ...
 */
function agent2wp_sign_upload_payload(array $payload): string|WP_Error
{
    ...
    $body = agent2wp_base64url_encode($json);
    $signature = hash_hmac('sha256', $body, agent2wp_upload_token_secret(), binary: true);

    return $body . '.' . agent2wp_base64url_encode($signature);
}
 ...
 */
function agent2wp_verify_upload_token(string $token): array|WP_Error
{
    ...
    $expected = agent2wp_base64url_encode(hash_hmac('sha256', $body, agent2wp_upload_token_secret(), binary: true));
    if (!hash_equals($expected, $signature)) {
        return new WP_Error('invalid_upload_token', 'Invalid upload token signature.', ['status' => 401]);
    }

    $json = agent2wp_base64url_decode($body);
    if ($json === false) {
 ...
 */
function agent2wp_handle_signed_upload(WP_REST_Request $request)
{
    if (!agent2wp_is_enabled()) {
        return new WP_Error('agent2wp_disabled', 'Agent2Wp abilities are disabled.', ['status' => 403]);
    }

    $token = agent2wp_get_upload_token_from_request($request);
    if ($token === '') {
        return new WP_Error('missing_upload_token', 'Missing upload token.', ['status' => 401]);
    }

    $payload = agent2wp_verify_upload_token($token);
    if (is_wp_error($payload)) {
        return $payload;
    }

    $destination = agent2wp_prepare_upload_destination($payload);
    if (is_wp_error($destination)) {
        return $destination;
    }

    $source = agent2wp_open_upload_source($request);
    ...
    $result = $destination['overwrite']
        ? agent2wp_overwrite_upload_stream(
            source: $stream,
            resolved: $destination['path'],
            max_bytes: $destination['max_bytes'],
        )
        : agent2wp_create_upload_stream(
            source: $stream,
            resolved: $destination['path'],
            max_bytes: $destination['max_bytes'],
        );
 ...
 */
function agent2wp_prepare_upload_destination(array $payload): array|WP_Error
{
    ...
    $resolved = agent2wp_resolve_path(path: $payload['path'], must_exist: false);
    if (is_wp_error($resolved)) {
        return $resolved;
    }

    $symlink_error = agent2wp_reject_final_path_symlink($resolved);
    ...
    $sandbox_error = agent2wp_check_php_execution_sandbox($resolved);
    ...
    if (!is_dir($parent_dir)) {
        ...
        $directories_created = agent2wp_ensure_parent_dir($parent_dir);
 ...
 */
function agent2wp_get_upload_token_from_request(WP_REST_Request $request): string
{
    return agent2wp_rest_header_token($request, header_name: 'x-agent2wp-upload-token');
}
 ...
 */
function agent2wp_open_upload_source(WP_REST_Request $request): array|WP_Error
{
    $file = agent2wp_get_multipart_upload_file($request);
    if ($file !== null) {
        return agent2wp_open_multipart_upload_source($file);
    }
 ...
 */
function agent2wp_get_multipart_upload_file(WP_REST_Request $request): ?array
{
 ...
 * @return array{stream: resource, source: string, filename: string}|WP_Error
 */
function agent2wp_open_multipart_upload_source(array $file): array|WP_Error
{
    $error = (int) ($file['error'] ?? UPLOAD_ERR_NO_FILE);
    if ($error !== UPLOAD_ERR_OK) {
        return new WP_Error('upload_failed', agent2wp_upload_error_message($error));
    }
 ...
 */
function agent2wp_create_upload_stream($source, string $resolved, int $max_bytes): array|WP_Error
{
    ...
    $bytes_written = agent2wp_copy_limited_stream(source: $source, target: $target, max_bytes: $max_bytes);
    fclose($target);
 ...
 * @return array{bytes_written: int, created: bool}|WP_Error
 */
function agent2wp_overwrite_upload_stream($source, string $resolved, int $max_bytes): array|WP_Error
{
    $created = !file_exists($resolved);
    $temporary_path = tempnam(dirname($resolved), prefix: '.agent2wp-upload-');
 ...
 * @return int|WP_Error
 */
function agent2wp_copy_limited_stream($source, $target, int $max_bytes): int|WP_Error
{
    $bytes_written = 0;
    while (!feof($source)) {
 ...
 */
function agent2wp_upload_error_message(int $error): string
{
    return match ($error) {
 ...
 */
function agent2wp_upload_token_secret(): string
{
    return wp_salt('auth') . '|' . wp_salt('secure_auth') . '|agent2wp-upload-link';
}

/**
 * Encode bytes with base64url.
 */
function agent2wp_base64url_encode(string $value): string
{
 ...
 */
function agent2wp_base64url_decode(string $value): string|false
{
