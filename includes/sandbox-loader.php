 ...
 */
function agent2wp_sandbox_crash_handler(string $crashed_file, ?string $current_sandbox_file): void
{
...
(static function () {
    $sandbox_dir = AGENT2WP_SANDBOX_DIR;
    ...
    $abilities_enabled = agent2wp_is_enabled();
    ...
    if (!$is_safe_mode && agent2wp_admin_request_flag('agent2wp_safe_mode')) {
        $is_safe_mode = true;
    }
    ...
    add_action('admin_notices', static function () use ($crashed_file) {
        ...
        if (file_exists($crashed_file)) {
            wp_admin_notice(
                sprintf(
                    '<strong>%s</strong> %s',
                    esc_html__('Agent2Wp Sandbox: Safe mode is active.', domain: 'agent2wp'),
                    esc_html__(
                        'A sandbox plugin caused a fatal error. All sandbox plugins are disabled. Fix or delete the broken plugin, then delete wp-content/agent2wp-sandbox/.crashed to resume.',
                        domain: 'agent2wp',
                    ),
                ),
                [
    ...
    register_shutdown_function(static function () use ($crashed_file, &$current_sandbox_file) {
        agent2wp_sandbox_crash_handler($crashed_file, $current_sandbox_file);
    });
