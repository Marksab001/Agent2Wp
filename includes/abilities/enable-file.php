...
wp_register_ability('agent2wp/enable-file', [
    'label' => __('Enable File', domain: 'agent2wp'),
    'description' => __(
    ...
    'input_schema' => [
        ...
        'properties' => [
            'path' => [
                'type' => 'string',
                'description' => 'Path to the .disabled file to re-enable. Can be the original filename (without .disabled suffix) or the full disabled filename. Relative paths are resolved from ABSPATH. Must be inside wp-content/agent2wp-sandbox/.',
                'minLength' => 1,
            ],
        ],
        'required' => ['path'],
    ...
    'execute_callback' => 'agent2wp_enable_file',
    ...
    'meta' => [
        ...
        'annotations' => [
            'instructions' => implode("\n", [
                'SANDBOX NOTES:',
                '- Only files inside wp-content/agent2wp-sandbox/ (the PHP sandbox) can be enabled.',
 ...
 */
function agent2wp_enable_file($input)
{
    $path = (string) $input['path'];

    // Normalize: ensure we are looking for the .disabled version of the file.
    $disabled_path = agent2wp_is_disabled_file($path) ? $path : $path . '.disabled';

    $resolved = agent2wp_resolve_path($disabled_path, must_exist: true);

    // If the .disabled file was not found, check whether the original file already exists (idempotent).
    if (is_wp_error($resolved) && !agent2wp_is_disabled_file($path)) {
        $original_resolved = agent2wp_resolve_path($path, must_exist: true);
    ...
    $sandbox_check = agent2wp_validate_sandbox_path($resolved);
    ...
    if (!agent2wp_is_disabled_file($resolved)) {
        return [
