...
wp_register_ability('agent2wp/create-admin-access-link', [
    'label' => __('Create Admin Access Link', domain: 'agent2wp'),
    'description' => __(
    ...
    'input_schema' => [
        ...
        'properties' => [
            ...
            'admin_path' => [
                'type' => 'string',
                'description' => 'Optional wp-admin-relative path to open after login, such as "plugins.php" or "admin.php?page=agent2wp-connect". External URLs are rejected.',
                'default' => '',
            ],
        ],
    ...
    'execute_callback' => 'agent2wp_create_admin_access_link',
 ...
 */
function agent2wp_create_admin_access_link(array $input = [])
{
    $user_id = get_current_user_id();
    if ($user_id <= 0 || !agent2wp_current_user_can_manage()) {
    ...
    $access = agent2wp_create_admin_access_token($user_id, $expires_in, $session_expires_in, $admin_path);
    if (is_wp_error($access)) {
        return $access;
    }

    $exchange_url = rest_url('agent2wp/v1/admin-access');
    $token_header = 'X-Agent2Wp-Admin-Access-Token';
    $nonce_header = 'X-Agent2Wp-Admin-Access-Nonce';
