...
wp_register_ability('agent2wp/write-file', [
    ...
    'execute_callback' => 'agent2wp_write_file',
    ...
    'meta' => [
        ...
        'annotations' => [
            'instructions' => implode("\n", [
                'PHP FILE SANDBOX:',
                '- PHP files (*.php) and PHP execution control files can ONLY be written to: wp-content/agent2wp-sandbox/',
                '- Use a path like "wp-content/agent2wp-sandbox/my-feature.php"',
                ...
                '- After fixing, delete the file "wp-content/agent2wp-sandbox/.crashed"',
 ...
 */
function agent2wp_decode_write_content(string $content, string $encoding): string|WP_Error
{
    if ($encoding === 'base64') {
 ...
 */
function agent2wp_write_file($input)
{
    $resolved = agent2wp_resolve_path(path: (string) $input['path'], must_exist: false);
    ...
    $content = agent2wp_decode_write_content((string) $input['content'], $encoding);
