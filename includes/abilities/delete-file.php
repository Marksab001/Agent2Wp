...
wp_register_ability('agent2wp/delete-file', [
    'label' => __('Delete File', domain: 'agent2wp'),
    'description' => __(
    ...
    'execute_callback' => 'agent2wp_delete_file',
    ...
    'meta' => [
        ...
        'annotations' => [
            'instructions' => implode("\n", [
                'SANDBOX NOTES:',
                '- Files in wp-content/agent2wp-sandbox/ (the PHP sandbox) can be deleted.',
                '- To exit safe mode after a crash, delete: wp-content/agent2wp-sandbox/.crashed',
            ]),
            'readonly' => false,
            'destructive' => true,
            'idempotent' => true,
 ...
 */
function agent2wp_delete_file($input)
{
    $resolved = agent2wp_resolve_path((string) $input['path'], must_exist: false);
    ...
    // Delete a directory.
    if (is_dir($resolved)) {
        return agent2wp_delete_directory($resolved, $recursive);
    }
 ...
 * @return array|WP_Error
 */
function agent2wp_delete_directory($resolved, $recursive)
{
