...
wp_register_ability('agent2wp/run-wp-cli', [
    'label' => __('Run WP-CLI Command', domain: 'agent2wp'),
    'description' => __(
    ...
    'execute_callback' => 'agent2wp_run_wp_cli',
...
wp_register_ability('agent2wp/get-wp-cli-job', [
    'label' => __('Get WP-CLI Job Status', domain: 'agent2wp'),
    'description' => __('Checks the status of an asynchronous background WP-CLI job.', domain: 'agent2wp'),
    'category' => 'code-execution',
    'input_schema' => [
        'type' => 'object',
        'properties' => [
    ...
    'execute_callback' => 'agent2wp_get_wp_cli_job',
    'permission_callback' => 'agent2wp_permission_callback',
    'meta' => [
        'show_in_rest' => true,
        'mcp' => ['public' => true],
 ...
 */
function agent2wp_run_wp_cli(array $input)
{
    ...
    foreach ($raw_args as $arg) {
        if (!is_string($arg)) {
            return new WP_Error('invalid_wp_cli_arg', __('WP-CLI arguments must be strings.', domain: 'agent2wp'));
        }
        $args[] = $arg;
    }
    ...
    $wp_path = agent2wp_find_wp_cli_path();
    if ($wp_path === null) {
    ...
    if (agent2wp_is_current_user_root() && !in_array(needle: '--allow-root', haystack: $args, strict: true)) {
        array_unshift($args, '--allow-root');
    }

    if ($async) {
        return agent2wp_run_wp_cli_async($wp_path, $args);
    }

    return agent2wp_run_wp_cli_sync($wp_path, $args);
}
 ...
 * @return array|WP_Error
 */
function agent2wp_get_wp_cli_job(array $input)
{
    $job_id = (string) ($input['job_id'] ?? '');
    if (!preg_match('/^[a-f0-9]{16}$/i', $job_id)) {
        return new WP_Error('invalid_job_id', __('Invalid job ID format.', domain: 'agent2wp'));
    }

    $job_dir = agent2wp_get_sandbox_dir(ensure_exists: false) . 'wp-cli-jobs/';
    $log_file = $job_dir . 'job_' . $job_id . '.log';
    $status_file = $job_dir . 'job_' . $job_id . '.status';

    if (!is_file($log_file)) {
        return agent2wp_get_missing_wp_cli_job_result($job_id, $status_file);
    }
    ...
    $slice = agent2wp_read_log_slice($log_file, $offset, $limit);
 ...
 * @return array
 */
function agent2wp_get_missing_wp_cli_job_result(string $job_id, string $status_file): array
{
    if (!is_file($status_file)) {
        return [
            'success' => false,
 ...
 */
function agent2wp_read_log_slice(string $log_file, int $offset, int $limit): array|WP_Error
{
    ...
    if ($handle === false) {
        return new WP_Error('read_failed', sprintf(__('Could not open log file: %s', domain: 'agent2wp'), $log_file));
    }
    ...
    if ($content === false) {
        return new WP_Error('read_failed', sprintf(__('Could not read log file: %s', domain: 'agent2wp'), $log_file));
    }
 ...
 * @return string|null
 */
function agent2wp_find_wp_cli_path(): ?string
{
 ...
 * @return bool
 */
function agent2wp_is_current_user_root(): bool
{
 ...
 * @return array
 */
function agent2wp_run_wp_cli_sync(string $wp_path, array $args): array
{
    ...
    if (!is_resource($process)) {
        return [
            ...
            'exit_code' => -1,
            'stdout' => '',
            'stderr' => __('Failed to start process.', domain: 'agent2wp'),
            'job_id' => null,
            'pid' => null,
        ];
    }
    ...
    [$stdout, $stderr] = agent2wp_collect_process_output(
 ...
 */
function agent2wp_collect_process_output(mixed $stdout_pipe, mixed $stderr_pipe): array
{
    ...
    while (is_resource($stdout_pipe) || is_resource($stderr_pipe)) {
        ...
        foreach ($read as $pipe) {
            agent2wp_append_process_pipe_output($pipe, $stdout_pipe, $stderr_pipe, $stdout, $stderr);
        }
    }
 ...
 */
function agent2wp_append_process_pipe_output(
    mixed $pipe,
    mixed $stdout_pipe,
    mixed $stderr_pipe,
    string &$stdout,
 ...
 * @return array
 */
function agent2wp_run_wp_cli_async(string $wp_path, array $args): array
{
    $job_id = bin2hex(random_bytes(8));
    $job_dir = agent2wp_get_sandbox_dir(ensure_exists: true) . 'wp-cli-jobs/';
    if (!is_dir($job_dir)) {
        wp_mkdir_p($job_dir);
    }
    ...
    if (file_put_contents(filename: $log_file, data: '') === false) {
        return [
            'success' => false,
            'stderr' => sprintf(__('Failed to create log file: %s', domain: 'agent2wp'), $log_file),
        ];
    }
    ...
    if ($rc !== 0 || $pid === null) {
        ...
        return [
            ...
            'exit_code' => $rc,
            'stdout' => '',
            'stderr' => __('Failed to start background WP-CLI process.', domain: 'agent2wp'),
        ];
    }
