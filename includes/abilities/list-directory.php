...
wp_register_ability('agent2wp/list-directory', [
    'label' => __('List Directory', domain: 'agent2wp'),
    'description' => __(
    ...
    'execute_callback' => 'agent2wp_list_directory',
    'permission_callback' => 'agent2wp_permission_callback',
    'meta' => [
        'show_in_rest' => true,
        'mcp' => ['public' => true],
        'annotations' => [
            'instructions' => 'TIP: AI-written PHP plugins live in wp-content/agent2wp-sandbox/. Check wp-content/agent2wp-sandbox/.crashed to see if safe mode is active.',
            'readonly' => true,
            'destructive' => false,
            'idempotent' => true,
        ],
 ...
 */
function agent2wp_list_directory(array $input = [])
{
    ...
    $resolved = agent2wp_resolve_path($path, must_exist: true);
    ...
    $all_entries = $recursive
        ? agent2wp_collect_recursive_entries($resolved, $pattern, $include_hidden, $max_depth)
        : agent2wp_collect_flat_entries($resolved, $pattern, $include_hidden);
 ...
 */
function agent2wp_collect_recursive_entries($resolved, $pattern, $include_hidden, $max_depth)
{
    $entries = [];
    ...
    foreach ($iterator as $item) {
        ...
        $entry = agent2wp_build_entry($item, $pattern, $include_hidden);
        if ($entry !== null) {
            $entries[] = $entry;
        }
    }
 ...
 */
function agent2wp_collect_flat_entries($resolved, $pattern, $include_hidden)
{
    ...
    while (($filename = readdir($dir_handle)) !== false) {
        ...
        $entry = agent2wp_build_entry($info, $pattern, $include_hidden);
        if ($entry !== null) {
            $entries[] = $entry;
        }
    }
 ...
 */
function agent2wp_build_entry($info, $pattern, $include_hidden)
{
