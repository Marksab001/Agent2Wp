...
namespace Agent2Wp\Abilities\Gutenberg;
...
const POST_TYPE = 'agent2wp_gb_change';

const KIND_BATCH = 'batch';

const KIND_ITEM = 'item';

const META_KIND = '_agent2wp_gb_kind';

const META_STATUS = '_agent2wp_gb_status';

const META_STATUS_UPDATED_AT = '_agent2wp_gb_status_updated_at';

const META_AGENT_LABEL = '_agent2wp_gb_agent_label';

const META_AGENT_SESSION_ID = '_agent2wp_gb_agent_session_id';

const META_READY_AT = '_agent2wp_gb_ready_at';

const META_FINALIZED_AT = '_agent2wp_gb_finalized_at';

const META_LEASE_OWNER = '_agent2wp_gb_lease_owner';

const META_LEASE_EXPIRES_AT = '_agent2wp_gb_lease_expires_at';

const META_LAST_ERROR = '_agent2wp_gb_last_error';

const META_TARGET_ID = '_agent2wp_gb_target_id';

const META_TARGET_TYPE = '_agent2wp_gb_target_type';

const META_OPERATION = '_agent2wp_gb_operation';

const META_BASE_CONTENT_HASH = '_agent2wp_gb_base_content_hash';

const META_BASE_CONTENT = '_agent2wp_gb_base_content';

const META_BASE_REVISION_ID = '_agent2wp_gb_base_revision_id';

const META_SPEC_HASH = '_agent2wp_gb_spec_hash';

const META_BLOCK_SPEC = '_agent2wp_gb_block_spec';

const META_VALIDATION_ERRORS = '_agent2wp_gb_validation_errors';

const META_FINALIZATION_MODE = '_agent2wp_gb_finalization_mode';

const META_FINALIZED_CONTENT = '_agent2wp_gb_finalized_content';
...
const FINALIZER_RUNTIME_TRANSIENT = 'agent2wp_gb_finalizer_runtimes';

const FINALIZER_RUNTIME_POLL_TOKEN_OPTION = 'agent2wp_gb_finalizer_poll_token';
...
add_action('agent2wp_gutenberg_cleanup', __NAMESPACE__ . '\\cleanup_queue');

function register_storage(): void
{
    register_post_type(POST_TYPE, [
        'label' => __('Gutenberg pending changes', domain: 'agent2wp'),
        'public' => false,
        'show_ui' => false,
        'show_in_rest' => false,
...
{
    if (wp_next_scheduled('agent2wp_gutenberg_cleanup') !== false) {
        return;
    }
    ...
    wp_schedule_event(
        ...
        recurrence: 'daily',
        hook: 'agent2wp_gutenberg_cleanup',
    );
}

function unschedule_cleanup(): void
{
    wp_clear_scheduled_hook('agent2wp_gutenberg_cleanup');
}
...
{
    $option = sprintf('agent2wp_gb_status_lock_%d', $post_id);
...
{
    $label = trim($label) !== '' ? sanitize_text_field($label) : __('Untitled Gutenberg batch', domain: 'agent2wp');
...
function target_title(WP_Post $target): string
{
    ...
    return $title !== '' ? $title : sprintf(__('(no title) #%d', domain: 'agent2wp'), $target->ID);
}
...
function validate_dynamic_only_blocks(array $blocks): ?WP_Error
{
    foreach ($blocks as $block) {
        ...
        if (!is_agent2wp_dynamic_block($name)) {
            return new WP_Error(
                'gutenberg_static_blocks_require_finalization',
                sprintf(
                    'Block "%s" is not a registered Agent2Wp-owned dynamic-only block. Native/static Gutenberg blocks must be queued with agent2wp/gutenberg-add-pending-change and finalized in a browser before they are live.',
                    $name !== '' ? $name : '(missing name)',
                ),
                [
                    'status' => 400,
                    'finalization_required' => true,
                    'queue_ability' => 'agent2wp/gutenberg-add-pending-change',
                    'enable_ability' => 'agent2wp/gutenberg-enable-batch-finalization',
                ],
            );
        }
...
function is_agent2wp_dynamic_block(string $name): bool
{
    if (!str_starts_with($name, 'agent2wp/')) {
        return false;
    }
...
{
    return add_query_arg(['page' => 'agent2wp-gutenberg-finalize'], admin_url('admin.php'));
}
...
function batch_label(WP_Post $batch): string
{
    ...
    return $label !== '' ? $label : sprintf(__('Gutenberg batch #%d', domain: 'agent2wp'), $batch->ID);
}
...
function user_instruction(WP_Post $batch): string
{
    ...
    if (($runtime['online'] ?? false) === true && ($runtime['can_finalize_batch'] ?? false) === true) {
        return sprintf(
            'The Agent2Wp Block Editor Queue page is open and should automatically finalize Gutenberg batch #%d: %s. Do not ask the user to do anything unless the page goes offline; stream %s with curl -N, or poll %s with curl, until the batch becomes finalized, failed, or conflicted. Do not treat these Gutenberg changes as live until finalization completes.',
            $batch->ID,
            batch_label($batch),
    ...
    if (($runtime['online'] ?? false) === true) {
        return sprintf(
            'A Agent2Wp Block Editor Queue page is open, but that browser user may not be able to finalize Gutenberg batch #%d: %s. Ask the user to open %s as a user who can edit every target. Do not treat these Gutenberg changes as live until finalization completes.',
            $batch->ID,
            batch_label($batch),
    ...
    return sprintf(
        'The Agent2Wp Block Editor Queue page is not currently online. Ask the user to open %s and keep it open. It will automatically finalize Gutenberg batch #%d: %s when it can. Do not treat these Gutenberg changes as live until finalization completes.',
        finalizer_dashboard_url(),
        $batch->ID,
        batch_label($batch),
    );
...
function shape_item(WP_Post $item): array
{
    ...
    return [
        ...
        'target_title' => $target instanceof WP_Post
            ? target_title($target)
            : sprintf(__('Missing target #%d', domain: 'agent2wp'), $target_id),
...
function conflict_payload(WP_Post $item): array
{
    ...
    return [
        ...
        'cancel_ability' => 'agent2wp/gutenberg-delete-pending-batch',
...
function claim_batch(int $batch_id): array|WP_Error
{
    ...
    if ($current_status === STATUS_DRAFT) {
        return new WP_Error(
            ...
            'Draft Gutenberg batches cannot be finalized until agent2wp/gutenberg-enable-batch-finalization is called.',
            ['status' => 409],
        );
    }
