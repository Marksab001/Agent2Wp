...
(function () {
    ...
    var hub = document.querySelector('.agent2wp-hub');
    ...
    hub.addEventListener('submit', function (event) {
        ...
        if (!(form instanceof HTMLFormElement) || !form.matches('.agent2wp-hub-actions form')) {
            return;
        }

        var actionInput = form.querySelector('input[name="agent2wp_ability_hub_action"]');
        if (!actionInput || actionInput.value !== 'toggle_disabled') {
            return;
        }

        var row = form.closest('.agent2wp-hub-row');
        ...
        body.set('action', 'agent2wp_toggle_ability');
    ...
    hub.addEventListener('click', function (event) {
        if (event.target instanceof HTMLElement && event.target.closest('.agent2wp-hub-select-all')) {
    ...
    hub.addEventListener('change', function (event) {
        ...
        if (input.classList.contains('agent2wp-hub-select-all-input')) {
            var section = input.closest('.agent2wp-hub-section, .agent2wp-hub-subsection');
            if (section) {
                section.querySelectorAll('.agent2wp-hub-row input[type="checkbox"]').forEach(function (box) {
        ...
        if (input.closest('.agent2wp-hub-row')) {
            syncSelectAllStates();
        }
    });
    ...
    function syncSelectAllStates() {
        hub.querySelectorAll('.agent2wp-hub-select-all-input').forEach(function (selectAll) {
            var section = selectAll.closest('.agent2wp-hub-section, .agent2wp-hub-subsection');
            if (!section) {
                return;
            }
            var boxes = section.querySelectorAll('.agent2wp-hub-row input[type="checkbox"]');
            var checked = 0;
    ...
    var bulkForm = document.getElementById('agent2wp-abilities-bulk');
    if (bulkForm) {
        bulkForm.addEventListener('submit', function (event) {
            if (selectedBulkAction() !== 'disable') {
                return;
            }
            var count = hub.querySelectorAll('.agent2wp-hub-row input[type="checkbox"]:checked').length;
            if (count === 0) {
                return;
            }
    ...
    function selectedBulkAction() {
        var top = document.getElementById('agent2wp-bulk-action-selector-top');
        var bottom = document.getElementById('agent2wp-bulk-action-selector-bottom');
    ...
    function applyToggle(row, button, data) {
        ...
        var subsection = row.closest('.agent2wp-hub-subsection');
        if (subsection) {
            refreshSectionMeta(subsection);
        }
        var section = row.closest('.agent2wp-hub-section');
        if (section) {
    ...
    // "All disabled" pill, mirroring agent2wp_render_ability_header_meta() in PHP.
    function refreshSectionMeta(section) {
        var summary = section.querySelector(':scope > summary');
        if (!summary) {
            return;
        }
        var rows = section.querySelectorAll('.agent2wp-hub-row');
        ...
        var pill = summary.querySelector('.agent2wp-hub-alloff');
        if (enabled === 0 && total > 0) {
            if (!pill && heading) {
                pill = document.createElement('span');
                pill.className = 'pill status is-disabled agent2wp-hub-alloff';
