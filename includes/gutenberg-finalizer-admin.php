...
namespace Agent2Wp\GutenbergFinalizer;
...
{
    return 'agent2wp-gutenberg-finalize';
}
...
function register_gutenberg_finalizer_menu(): void
{
    ...
    add_submenu_page(
        ...
        page_title: __('Block Editor Queue', domain: 'agent2wp'),
        menu_title: __('Block Editor Queue', domain: 'agent2wp'),
...
function enqueue_gutenberg_finalizer_assets(string $hook_suffix): void
{
    ...
    if (is_string($encoded_config)) {
        wp_add_inline_script(
            handle: 'agent2wp-gutenberg-finalizer',
            data: 'window.agent2wpGutenbergFinalizer = ' . $encoded_config . ';',
            position: 'before',
        );
    }
    wp_add_inline_script(handle: 'agent2wp-gutenberg-finalizer', data: gutenberg_finalizer_script());
    wp_enqueue_script(handle: 'agent2wp-gutenberg-finalizer');
...
{
    return agent2wp_admin_request_page() === gutenberg_finalizer_page_slug();
}
...
function render_gutenberg_finalizer_page(): void
{
    ...
    ?>
    <div class="wrap agent2wp-gb-finalizer" id="agent2wp-gb-finalizer">
        <h1 class="wp-heading-inline"><?php esc_html_e('Block Editor Queue', domain: 'agent2wp'); ?></h1>
...
{ ?>
    <div id="agent2wp-gb-notice" class="notice" hidden><p></p></div>

    <section class="summary-panel" aria-live="polite">
        <p><?php esc_html_e(
            'This background utility page is used by Agent2Wp to safely validate and serialize Gutenberg blocks. During Gutenberg editing sessions, this page serves as a technical bridge, utilizing the native WordPress editor engine to serialize block structures securely.',
            domain: 'agent2wp',
        ); ?></p>
        ...
        <p id="agent2wp-gb-progress" class="progress-line"><?php esc_html_e(
            'Checking for queued Gutenberg changes...',
            domain: 'agent2wp',
        ); ?></p>
    </section>
    <div class="agent2wp-gb-editor-frame-wrap" aria-hidden="true">
        <iframe
            id="agent2wp-gb-editor-frame"
            class="agent2wp-gb-editor-frame"
            title="<?php esc_attr_e('Agent2Wp hidden block editor', domain: 'agent2wp'); ?>"
            tabindex="-1"
            src="about:blank"
        ></iframe>
    </div>
...
{ ?>
    <style>
        .agent2wp-gb-finalizer .summary-panel {
        ...
        .agent2wp-gb-finalizer .summary-panel p {
            font-size: 14px;
            line-height: 1.6;
            color: #4a5568;
            margin: 0 0 12px 0;
        }
        .agent2wp-gb-finalizer .summary-panel p strong {
            color: #2d3748;
        }
        .agent2wp-gb-finalizer .progress-line {
            display: flex;
            align-items: center;
            gap: 10px;
            margin: 20px 0 0 0;
        ...
        .agent2wp-gb-finalizer .progress-line::before {
            content: "";
        ...
        .agent2wp-gb-finalizer .agent2wp-gb-editor-frame-wrap {
            position: absolute;
            top: 0;
            left: -10000px;
            width: 1280px;
        ...
        .agent2wp-gb-finalizer .agent2wp-gb-editor-frame {
            display: block;
            width: 1280px;
            height: 900px;
            border: 0;
...
function gutenberg_finalizer_script(): string
{
    return <<<'JS'
        ( function () {
            const config = window.agent2wpGutenbergFinalizer || {};
            const root = document.getElementById( 'agent2wp-gb-finalizer' );
            ...
            const progress = document.getElementById( 'agent2wp-gb-progress' );
            const notice = document.getElementById( 'agent2wp-gb-notice' );
            const editorFrame = document.getElementById( 'agent2wp-gb-editor-frame' );
            ...
            const path = ( suffix ) => `/agent2wp/v1${ suffix }`;
